...
class Camera2VideoFragment : Fragment(), View.OnClickListener,
    ...
    private fun openCamera(width: Int, height: Int) {
        ...
        try {
            ...
            //makes the camera front facing
            val cameraIds = manager.cameraIdList
            var cameraId: String? = null
            for (poscameraId in cameraIds) {
                cameraCharacteristics = manager.getCameraCharacteristics(poscameraId)
                val temp = cameraCharacteristics
                if (CameraMetadata.LENS_FACING_FRONT == temp?.get(CameraCharacteristics.LENS_FACING)!!.toInt()) {
                    Log.i("MainActivity", String.format("%s: %s", "findFrontFacingCameraId", poscameraId))
                    cameraId = poscameraId
                }
            }
...
//            ErrorDialog.newInstance(getString(R.string.camera_error)).show(childFragmentManager, FRAGMENT_DIALOG)
     ...
     */
    private fun chooseOptimalSize(choices: Array<Size>, width: Int, height: Int, aspectRatio: Size):
        Size {
